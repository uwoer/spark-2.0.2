...
class DAGScheduler(
  ...
  /**
    * add by uwoer
    * 获取父stage的算法
    * 对一个stage如果它的最后一个rdd的所有依赖都是窄依赖，则不会创建任何新的stage
    * 但是 只要发现了者rdd宽依赖了某个rdd,那么就用宽依赖的那个rdd创建一个新的stage 并返回
    */
  private def getMissingParentStages(stage: Stage): List[Stage] = {
        ...
        if (rddHasUncachedPartitions) {
          ...
          for (dep <- rdd.dependencies) {
            dep match {
              //如果是宽依赖
              case shufDep: ShuffleDependency[_, _, _] =>
                //使用宽依赖的那个rdd创建一个ShuffleMapStage 并将shuffleMap设置为true
                //默认最后一个stage 不是shuffleMap的stage
                //但是finalStage之前所有的stage都是shuffleMap的stage
    ...
    //首先向栈中推入了stage的最后一个rdd
    waitingForVisit.push(stage.rdd)

    //然后进行while循环
    while (waitingForVisit.nonEmpty) {
      //对stage 的最后一个rdd调用上面定义的内部方法visi()
      visit(waitingForVisit.pop())
    }
    missing.toList
  }
  ...
  def submitJob[T, U](
    ...
    // add by uwoer
    // 发布job提交事件 DAGSchedulerEventProcessLoop 的 onReceive()方法中会进行事件的响应处理
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    try {
      ...
      // 1.以job的最后一个rdd创建finalStage 并加入到DAGScheduler内部的内存缓存中
    ...
    //4.使用submitStage()方法尝试提交finalStage
    //其实会导致第一个stage的提交 并且其他所有的stage都在waitingStages队列中等待调度
    submitStage(finalStage)

    //提交waitingStages队列中stage
    submitWaitingStages()
  }
  ...
  private def submitStage(stage: Stage) {
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        //调用getMissingParentStages的方法去获取当前这个stage的父stage
        val missing = getMissingParentStages(stage).sortBy(_.id)
        logDebug("missing: " + missing)

        //反复的递归调用一直到最初的stage(它没有父stage)
        //提交（第一个stage）最初的stage(它没有父stage) stage0
        //其他的stage此时都在waitingStages里等待调度
        if (missing.isEmpty) {
          ...
          //将当前stage放入waitingStages等待执行的stage队列中
          waitingStages += stage
        }
      }
    } else {
  ...
  /**
    * add by uwoer
    *提交stage,为stage创建task,taks数据与partition的数量相等
    */
  private def submitMissingTasks(stage: Stage, jobId: Int) {
      ...
      stage match {
        ...
        case s: ResultStage =>
          ...
          partitionsToCompute.map { id =>
            ...
            //getPreferredLocs()计算每个task对应partition的最佳位置
    ...
    val tasks: Seq[Task[_]] = try {
      stage match {
        ...
        //除了finalStage之外其他的所有stage都是ShuffleMapStage，finalStage创建ResultTask
        case stage: ResultStage =>
    ...
    if (tasks.size > 0) {
      ...
      //针对stage的task，创建TaskSet对象，调用TaskScheduler的submitTasks()方法，提交TaskSet
      //standalone模式下使用TaskSchedulerImpl
  ...
  /**
    * add by uwoer
    * 计算每个task对应partition的最佳位置
    * 从stage的最后一个rdd开始去找哪个rdd的partition是被cache或者checkpoint了的。
    * task的最佳位置就是cache或者checkpoint的partition的位置
    * 这样task在那个节点上执行就不需要计算之前的rad了
    */
  private def getPreferredLocsInternal(
    ...
    //查看当前rdd的partition是否checkpoint了
    ...
    //递归调用自己，查找rdd的父rdd是否cache或者checkpoint
    ...
    //如果这个stage从最后一个rdd到最开始的rdd中的partition都没有cache或者checkpoint 则该task的最佳位置为Nil 即没有最佳位置
    Nil
  }
